    ...
    async submitSubscriptionRequest() {
      try {
        ...
        if (!this.selectedPlan || !this.selectedPaymentMethod) {
          throw new Error('Please select both a plan and payment method');
        }
        
        await axios.post('/api/subscription-requests', {
        ...
        this.error = error.response?.data?.message || 'Failed to submit subscription request. Please try again.';
      } finally {
        this.loading = false;
      }
    },
